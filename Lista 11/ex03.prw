...
Static Function CriaRel(cAlias)
    ...
    Local cArquivo := "RelatorioPedCompra.pdf"
...
Static Function ImpDados(cAlias)
    ...
    oPrint:Say(nLinha+=20,405, AllTrim((cAlias)->(C7_LOJA)), oFont12,,nCor)
    ...
    oPrint:Say(nLinha+=20,330, TRANSFORM(DtoC(StoD((cAlias)->(C7_EMISSAO))), "@R 99/99/9999"), oFont12,,nCor)
    
    oPrint:Say(nLinha,20, "Cond. Pagamento:", oFont14,,nCor)
    oPrint:Say(nLinha+=20,105, AllTrim((cAlias)->(C7_COND)) + " - " + AllTrim((cAlias)->(E4_DESCRI)), oFont12,,nCor)
